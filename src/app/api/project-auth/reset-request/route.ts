...
/**
 * POST /api/project-auth/reset-request
 * Body: { email: string }
...
export async function POST(req: NextRequest) {
  try {
    ...
    await dbSet("project_auth_reset_otp", { otp, expiresAt });
    ...
    if (sendError) {
      console.error("[project-auth reset-request] Resend error:", sendError);
      return NextResponse.json(
    ...
    console.error("[project-auth reset-request] error:", err);
    return NextResponse.json(
