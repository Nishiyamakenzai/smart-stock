...
/**
 * POST /api/project-auth/setup
 * Body: { id: string, pw: string }
 * 案件進捗管理システム専用アカウントの初回セットアップ（COATEX・TASUKIとは別管理）
 */
export async function POST(req: NextRequest) {
  ...
  const existing = await dbGet("project_auth");
  if (existing) {
    return NextResponse.json({ error: "アカウントは既に設定済みです" }, { status: 409 });
  }

  const passwordHash = await bcrypt.hash(pw, 10);
  await dbSet("project_auth", { id, passwordHash });
