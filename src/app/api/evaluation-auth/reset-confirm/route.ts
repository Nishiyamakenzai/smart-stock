...
/**
 * POST /api/evaluation-auth/reset-confirm
...
export async function POST(req: NextRequest) {
  try {
    ...
    const stored = await dbGet<{ otp: string; expiresAt: number }>("evaluation_auth_reset_otp");
    ...
    try {
      await dbDelete("evaluation_auth_reset_otp");
    } catch {
      // 削除失敗しても続行（期限切れで再利用不可）
    }

    const passwordHash = await bcrypt.hash(newPw, 10);
    await dbSet("evaluation_auth", { id: newId, passwordHash });
    ...
    console.error("[evaluation-auth reset-confirm] error:", err);
